...
## List of useful Linux aliases
#updatef and cleanf must be executed as 'Superuser$'
#updatef Will check for updates for normal software installed and also for your distribution and after all
#cleanf will clean trash that you don't really need and will check if any installation has gone through any error.
alias updatef='echo Downloading software && echo  && apt-get update && echo Updating distribution && echo  && apt dist-upgrade && echo  && echo Upgrading software && echo  && apt-get upgrade'
alias cleanf='echo Cleaning trash... && rm -rf /tmp/* && echo  && apt-get autoclean && echo  && echo Removing and checking failed installations && echo  && apt-get autoremove && apt-get clean && apt-get check && apt install -f'

# For all the update and upgrades and after that cleans your system. Updatef + cleanf all in one command
alias updateff='updatef && cleanf'

## Get rid of command not found ##
alias cd..='cd ..'

## A quick way to move backwards from the current directory ##
alias ..='cd ..'
alias ...='cd ../../'
alias ....='cd ../../../'
alias .....='cd ../../../../'

## Use only 'extract' for most of your compressed files. ##
extract () {
   if [ -f "$1" ] ; then
       case "$1" in
           *.tar.bz2)   tar xvjf "$1"    ;;
       ...
       echo "$1 is not a valid file, or has spaces in it!"
   fi 
 } 
...
alias gtp+='git push'
alias gtp-='git pull'
alias gtl='git log --color --graph --pretty=format:"%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset" --abbrev-commit --'
alias gtshow='git show'
alias gtstash='git stash'
alias gtinit='git init'

#Create GitHub repository from terminal using GitHub API, with curl
gtCreate () {
    cancel=false
    unset email
    unset name
    ...
    echo "We will create a GitHub repository from scratch' you will need to have installed 'curl' and 'git'. "
    echo 

    while [ -z "$email" ]; do
        read -p 'Username or email address of your Github account: ' email
    done
    while [ -z "$name" ]; do
        read -p 'Name of the new repository: ' name
    done
    while [ -z "$description" ]; do
        read -p 'Description of the new repository: ' description
    done

    read -p "Do you wish to make the repository private?[Yy/Nn]: " yn
    case $yn in
    [Yy]* )
        curl -u "$email" https://api.github.com/user/repos -d '{"name":"'"$name"'","private":"true","description":"'"$description"'"}'
        ;;
    [Nn]* )
        curl -u "$email" https://api.github.com/user/repos -d '{"name":"'"$name"'","description":"'"$description"'"}'
        ;;
    * ) echo 'Please answer yes or no. Your installation will be aborted. Try again'
        cancel=true
        ;;
    esac

    #creating local folder and initializing git
    if [ $cancel == false ]; then
    	mkdir -pv "$name"
    	if cd "$name"; then echo "Ok"; else echo "Fail when executing cd $name" exit; fi
    	git init
    fi
}
...
clines () { 
	if [ -z "$1" ]; then
		echo 'At least give one file type by parameter'
	elif  [ -z "$2" ]; then
		find -type f -name "*.$1" -o -name "*.$2" | xargs wc -l | sort -n
	elif  [ -z "$3" ]; then
		find -type f -name "*.$1" -o -name "*.$2" -o -name "*.$3" | xargs wc -l | sort -n
	elif  [ -z "$4" ]; then
		find -type f -name "*.$1" -o -name "*.$2" -o -name "*.$3" -o -name "*.$4" | xargs wc -l | sort -n
	elif  [ -z "$5" ]; then
		find -type f -name "*.$1" -o -name "*.$2" -o -name "*.$3" -o -name "*.$4" -o -name "*.$5" | xargs wc -l | sort -n
	fi
} 
...
# might fix some sudo + alias problems
alias sudo='sudo '

# An alias to save some milliseconds of your time :)
alias sudos='sudo -s'
alias suspend='systemctl suspend'
alias hibernate='systemctl hibernate'
alias defrag='sudo e4defrag -vc '"$1"''
alias sysinf='cat /etc/os-release'
alias ram='free -h'
dusage() {
	du -hsBM "$1"* | sort -n
}
...
#Get process that cosumes most cpu
alias pscpu="ps auxf | sort -nr -k 3"
alias pscpu10="ps auxf | sort -nr -k 3 | head -10"

#Get process that cosumes most memory
alias psmem="ps auxf | sort -nr -k 4"
alias psmem10="ps auxf | sort -nr -k 4 | head -10"

# Get what is consuming the most disk space, icluding directories and files.
